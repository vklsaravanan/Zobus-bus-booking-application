...
public class SearchBusDAO {
	public static List<SearchEndPointModel> searchBus(String source, String destination, String date_of_week) {
        Connection connection = null;
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
        List<SearchEndPointModel> buses = new ArrayList<>(); 
        try { 
            ...
            String sql = "SELECT " +
                    ...
                    "    Buses.bus_name, " +
                    "    Buses.total_seats, " +
                    "    BusRoutes.departure_date_time, " +
                    "    BusRoutes.arrival_date_time, " +
                    "    BusRoutes.fare " +
                    "FROM " +
                    "    Routes " +
                    "JOIN " +
                    "    BusRoutes ON Routes.route_id = BusRoutes.route_id " +
                    "JOIN " +
                    "    Buses ON BusRoutes.bus_id = Buses.bus_id " +
                    "WHERE " +
                    "    Routes.source = ? " +
                    "    AND Routes.destination = ? " +
                    "    AND FIND_IN_SET(?, BusRoutes.day_of_week) > 0";
            
            preparedStatement = connection.prepareStatement(sql);
            
            preparedStatement.setString(1, source );
            preparedStatement.setString(2, destination );
            // here need to convert date into any time because search only by date but time stamp contain time also
            preparedStatement.setInt(3, Integer.valueOf(date_of_week)); 
            resultSet = preparedStatement.executeQuery();
            
            while (resultSet.next()) {
                SearchEndPointModel bus = new SearchEndPointModel();
